...
----
+
Starting with **RHOSO 18.0.1**, the neutron-ovn-metadata-agent uses the system journal instead of traditional log files. If you are using **RHOSO 18.0.1 or later**, use `journalctl` to check the logs.
+
----
[root@compute01 ~]# journalctl --unit edpm_ovn_metadata_agent.service
----
+
